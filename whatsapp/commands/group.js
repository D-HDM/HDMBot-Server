...
function getCountry(num) {
    const countries = {
        ...
        '971': '🇦🇪 UAE', '966': '🇸🇦 Saudi Arabia', '20': '🇪🇬 Egypt', '61': '🇦🇺 Australia'
    };
...
function register(commands, categories, category) {
    ...
    commands.set('antilink', {
        ...
        execute: async (sock, msg, args, config) => {
            ...
            if (!action || !state_) {
                ...
                return await sock.sendMessage(groupJid, { text: `🛡️ Anti-Link: ${current.enabled ? 'ON' : 'OFF'} | ${current.action}\n${config.PREFIX}antilink <delete|kick|warn> <on|off>` });
            }
    ...
    // ========== LISTBADWORD (Anyone, group + private) ==========
    commands.set('listbadword', {
        category,
        desc: 'List bad words',
        aliases: ['badwords', 'listbadwords'],
        execute: async (sock, msg, args, config) => {
            const chatJid = getChatJid(msg);
            const words = state.getBadWords(chatJid);
            if (!words.length) return await sock.sendMessage(chatJid, { text: '📋 No bad words.' });
            await sock.sendMessage(chatJid, { text: `🚫 *Bad Words (${words.length})*\n${words.sort().join(', ')}` });
        }
    });

    // ========== ADDBADWORD (Anyone, group + private) ==========
    commands.set('addbadword', {
        ...
        execute: async (sock, msg, args, config) => {
            ...
            if (!word) return await sock.sendMessage(msg.key.remoteJid, { text: `❌ Usage: ${config.PREFIX}addbadword <word>` });
            const chatJid = getChatJid(msg);
            state.addBadWord(chatJid, word);
            await sock.sendMessage(chatJid, { text: `✅ "${word}" added.` });
        }
    });

    // ========== REMOVEBADWORD (Anyone, group + private) ==========
    commands.set('removebadword', {
        category,
        desc: 'Remove bad word',
        aliases: ['delword', 'rmword'],
        execute: async (sock, msg, args, config) => {
            const word = (args[0] || '').toLowerCase();
            if (!word) return await sock.sendMessage(msg.key.remoteJid, { text: `❌ Usage: ${config.PREFIX}removebadword <word>` });
            const chatJid = getChatJid(msg);
            const removed = state.removeBadWord(chatJid, word);
            await sock.sendMessage(chatJid, { text: removed ? `✅ "${word}" removed.` : '❌ Not found.' });
        }
    });

    // ========== SETWARN (Anyone, group + private) ==========
    commands.set('setwarn', {
        ...
        execute: async (sock, msg, args, config) => {
            ...
            if (isNaN(limit) || limit < 1 || limit > 10) return await sock.sendMessage(msg.key.remoteJid, { text: '❌ Provide limit 1-10.' });
            const chatJid = getChatJid(msg);
            state.setWarningLimit(chatJid, limit);
            await sock.sendMessage(chatJid, { text: `✅ Warning limit set to ${limit}.` });
        }
    });
    ...
    commands.set('antistatusmention', {
        ...
        execute: async (sock, msg, args, config) => {
            ...
            if (!action || !state_) {
                ...
                return await sock.sendMessage(groupJid, { text: `📵 Anti-Status: ${current.enabled ? 'ON' : 'OFF'} | ${current.action}\n${config.PREFIX}antistatusmention <delete|kick|warn> <on|off>` });
            }
            if (!['delete', 'kick', 'warn'].includes(action)) return await sock.sendMessage(groupJid, { text: '❌ Action: delete, kick, warn' });
            if (!['on', 'off'].includes(state_)) return await sock.sendMessage(groupJid, { text: '❌ State: on, off' });
    ...
    commands.set('kickall', {
        ...
        execute: async (sock, msg, args, config) => {
            ...
            for (let i = 0; i < nonAdmins.length; i += 20) {
                ...
                try { await sock.groupParticipantsUpdate(groupJid, batch, 'remove'); kicked += batch.length; } catch {}
    ...
    commands.set('groupdesc', {
        ...
        aliases: ['desc', 'setdesc', 'description'],
        execute: async (sock, msg, args, config) => {
            if (!isGroup(msg)) return await sock.sendMessage(msg.key.remoteJid, { text: '❌ Groups only.' });
            const groupJid = msg.key.remoteJid;
            const senderJid = msg.key.participant || msg.key.remoteJid;
            ...
            if (!args.length) {
                try {
                    ...
                    return await sock.sendMessage(groupJid, { text: '❌ Failed to get description.' });
                }
            }
